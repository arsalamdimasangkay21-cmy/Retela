...
DROP TABLE IF EXISTS pos_transaction_logs;
...
CREATE TABLE apparel_items (
  ...
  category VARCHAR(80) NOT NULL DEFAULT 'T-shirt',
  gender VARCHAR(80) DEFAULT 'Unisex',
  size VARCHAR(80) DEFAULT 'Free Size',
  color VARCHAR(80) NOT NULL DEFAULT 'Unspecified',
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE INDEX idx_apparel_items_sku (sku),
...
CREATE TABLE pos_transaction_logs (
  id INT AUTO_INCREMENT PRIMARY KEY,
  order_id INT NOT NULL,
...
ALTER TABLE notifications
  ADD CONSTRAINT fk_notifications_broadcast FOREIGN KEY (broadcast_id) REFERENCES broadcasts(id) ON DELETE SET NULL;
...
VALUES
('admin', 'admin@retela.local', '$2a$12$/v7ZPMocx86ByjSVFeiW6.VF8xe/HqsHfXUYsCuV5KxCQm1uVSrw6', 'admin', 'approved', true),
('AdministratorRetela', 'administrator@retela.local', '$2a$12$K.QOgyK6w1cQPqS7HSRjW.mSzKhRLaxL0oN4BIzkPe68/1X.U354W', 'admin', 'approved', true);
