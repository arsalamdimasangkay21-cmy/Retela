...
export function Button({ children, className = "", variant = "primary", ...props }) {
  ...
  return (
    <button className={`${styles} inline-flex min-w-0 items-center justify-center gap-2 rounded-2xl px-4 py-2.5 text-center text-sm font-semibold transition active:scale-95 disabled:cursor-not-allowed disabled:opacity-60 ${className}`} {...props}>
      {children}
    </button>
  );
}
  ...
  return (
    <Card className="group overflow-hidden">
      <Wrapper type={onClick ? "button" : undefined} onClick={onClick} className={`flex min-w-0 items-center justify-between gap-4 text-left ${onClick ? "w-full cursor-pointer" : ""}`}>
        ...
        <span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl border border-emerald-100 bg-emerald-50 text-emerald-700 transition group-hover:scale-105">
          <Icon size={24} />
        </span>
      </Wrapper>
    </Card>
...
export function Field({ icon: Icon, invalid = false, wrapperClassName = "", className = "", ...props }) {
  return (
    <label className={`flex min-w-0 items-center gap-3 rounded-2xl border bg-white px-3 py-2.5 shadow-sm transition focus-within:ring-4 ${invalid ? "border-red-500 focus-within:border-red-500 focus-within:ring-red-100" : "border-slate-200 focus-within:border-emerald-300 focus-within:ring-emerald-100"} ${wrapperClassName}`}>
