...
router.get("/", requireAuth, asyncHandler(async (req, res) => {
  ...
  const rows = await query(
    `SELECT r.*, u.username, o.total_amount, o.status AS order_status,
       COALESCE(NULLIF(GROUP_CONCAT(DISTINCT p.name ORDER BY p.name SEPARATOR ', '), ''), CONCAT('Order #', r.order_id)) AS product_names,
       SUBSTRING_INDEX(GROUP_CONCAT(p.image_url ORDER BY oi.id SEPARATOR '||'), '||', 1) AS product_image
     FROM returns r
